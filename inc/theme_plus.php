...
{
  return rest_url('aurore/v1/image/feature') . '?' . rand(1, 1000);
}
...
{
  $chunks = array(
    array(24 * 60 * 60, __('天前', 'aurore')),
    array(60 * 60, __('小时前', 'aurore')),
    array(60, __('分钟前', 'aurore')),
    array(1, __(' 秒前', 'aurore'))
  );
              ...
              <section class="commeta">
                <div class="left">
                  <h4 class="author"><a href="<?php comment_author_url(); ?>"><?php echo get_avatar($comment->comment_author_email, '80', '', get_comment_author()); ?><?php comment_author(); ?> <span
                        class="isauthor" title="<?php esc_attr_e('作者', 'aurore'); ?>"></span></a></h4>
                </div>
                <div class="right">
...
{ ?>
  <script language='javascript' type='text/javascript'>
    ...
    function gopage(url, type) {
      URL = url;
      if (type == 1) {
        TYPE = <?php _e('管理后台', 'aurore') ?>;
      } else {
        TYPE = <?php _e('主页', 'aurore')?>;
      }
    ...
    function doUpdate(num) {
      document.getElementById('login-showtime').innerHTML = '<?php _e("空降成功, ", "aurore")?>' + num + '<?php _e("秒后自动转到", "aurore") ?>' + TYPE;
...
function login_ok()
{
  ...
  <p class="ex-logout">
    <a href="<?php bloginfo('url'); ?>" title="<?php _e('首页', 'aurore')?>"><?php _e('首页', 'aurore') ?></a>
    <?php if (current_user_can('level_10')) { ?>
      <a href="<?php bloginfo('url'); ?>/wp-admin/" title="<?php _e('后台', 'aurore')?>" target="_top"><?php _e('后台', 'aurore')?></a>
    <?php } ?>
    <a href="<?php echo wp_logout_url(get_bloginfo('url')); ?>" title="<?php _e('登出', 'aurore')?>"
      target="_top"><?php _e('登出？? ', 'aurore') ?></a>
  </p>
<?php
}
...
function the_video_headPattern_hls()
{
  ...
  if (is_single()) {
    ...
    $full_image_url = get_random_bg_url();
    $thubm_image_url = 'https://cdn.jsdelivr.net/gh/moezx/cdn@3.0.1/img/svg/loader/orange.progress-bar-stripe-loader.svg';
    $t .= '<h1 class="entry-title search-title"> ' . sprintf(__("关于 \" %s \"的搜索结果", "aurore"), get_search_query()) .'</h1>';
  }
  ...
  if (is_single()) {
    ...
    if (have_posts()):
      while (have_posts()):
        ...
        global $user_ID;
        if ($user_ID && current_user_can('level_10')) {
          $edit_this_post_link = '<span class="bull">·</span><a href="' . get_edit_post_link() . '">' . _e("编辑", "aurore") . '</a>';
        } else {
          $edit_this_post_link = '';
        }
        $t .= the_title('<h1 class="entry-title">', '<button id="coverVideo-btn" class=".constant-width-to-height-ratio" onclick="coverVideo()"><i class="fa fa-pause" aria-hidden="true"></i></button></h1>', false);
        $t .= '<p class="entry-census"><span><a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'), get_the_author_meta('user_nicename'))) . '"><img src="' . get_avatar_url(get_the_author_meta('ID'), 64) /*$ava*/. '"></a></span><span><a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'), get_the_author_meta('user_nicename'))) . '">' . get_the_author() . '</a></span><span class="bull">·</span>' . poi_time_since(get_post_time('U', true), false, true) . '<span class="bull">·</span>' . get_post_views(get_the_ID()) . ' ' . _n("View", "次阅读", get_post_views(get_the_ID()), "aurore") /*次阅读*/. $edit_this_post_link . '</p>';
      endwhile;
    endif;
  } elseif (is_page()) {
    $full_image_url = $full_image_url[0];
    ...
    <div class="header-user-avatar">
      ...
      <div class="header-user-menu">
        ...
        <div class="user-menu-option">
          <?php if (current_user_can('level_10')) { ?>
            <a href="<?php bloginfo('url'); ?>/wp-admin/" target="_blank"><?php _e('管理中心', 'aurore') ?></a>
            <a href="<?php bloginfo('url'); ?>/wp-admin/post-new.php" target="_blank"><?php _e('撰写文章', 'aurore') ?></a>
          <?php } ?>
          <a href="<?php bloginfo('url'); ?>/wp-admin/profile.php" target="_blank"><?php _e('个人资料', 'aurore') ?></a>
          <a href="<?php echo wp_logout_url(get_bloginfo('url')); ?>" target="_top"><?php _e('退出登录', 'aurore') ?></a>
        </div>
      </div>
    </div>
    <?php
  } else {
    $ava = 'https://cdn.yeyufan.cn/blog/img/none.png';
...
function siren_private_message_hook($comment_content, $comment)
{
  ...
  if ($is_private)
    return '<i class="fa fa-lock" aria-hidden="true"></i> ' . __("该评论为私密评论", "aurore");
  return $comment_content;
}
